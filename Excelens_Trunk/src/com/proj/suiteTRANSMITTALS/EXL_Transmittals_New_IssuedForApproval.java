...
public class EXL_Transmittals_New_IssuedForApproval extends TestSuiteBase{
	...
	private static String workflow_actions="Level-2/3:-Validating Associated Actions ";
	private static String workflow_documentRegister="Document Register : Validating Approval Status ";
	...
	public static void TestEXL_Transmittals_New_IssuedForApproval(Hashtable<String,String>data
			) throws Throwable{
		...
		try{
			...
			for (int userIteration=1 ;userIteration<= Transmittals_EntryPage.getRecieverUserCount(data);userIteration++){				
				...
				if (userIteration ==1)
				{
					...
					username = Constants_ConfigProperties.username_BusinessOwner;
					password = Constants_ConfigProperties.password_BusinessOwner;

				}
				else if (userIteration ==2)
				{
					username = Constants_ConfigProperties.username_NonAdminUser;
					password = Constants_ConfigProperties.password_NonAdminUser;
			...
			String attachDocs_approvalStatus;
			if(data.get("Action-Level2").equalsIgnoreCase("Rejected")){
				attachDocs_approvalStatus="Rejected";
				
			}else{
				attachDocs_approvalStatus="Approved";
			}
			DocumentRegistryPage.documentRegister_AttachdocumentAndSupportDocument_ApprovalStatus(driver_TRANS, workflow_documentRegister, data.get("AttachDocumentName"),data.get("AttachSupportDocumentName"), attachDocs_approvalStatus,"Being Approved");
