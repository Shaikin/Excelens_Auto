...
import com.proj.utilApp.WorkArounds;
...
public class Transmittals_EntryPage extends TestSuiteBase{
	...
	public static void switchToTramsmittalViewFrame1(WebDriver driver,String refID,String testcaseName,String workFlow) throws Throwable{
	 ...
	 * @throws Throwable 
	 */
	public static void clickSend(WebDriver driver,String workFlow) throws Throwable{
		ApplicationMethods.switchToLatestDLGframe(driver, testcaseName);
		...
		WaitUtil.pause(6);
		boolean isSendDisplayed=false;
		int recoveryCounter=0;
		isSendDisplayed=ExplicitWaitUtil.waitUntilInvisibilityOfElement(driver, objects_locatorType_Transmittals.get("Tramsmittals-Send"), objects_objectLocator_Transmittals.get("Tramsmittals-Send"), Constants_TimeOuts.Element_TimeOut);
		while(recoveryCounter<=3 && isSendDisplayed==false){
			ApplicationMethods.switchToLatestDLGframe(driver, testcaseName);
			isSendDisplayed=ElementPresence.isElementDisplayed(driver, objects_locatorType_Transmittals.get("Tramsmittals-Send"), objects_objectLocator_Transmittals.get("Tramsmittals-Send"), Constants_TimeOuts.sync_element_load);
			if(isSendDisplayed==true){
				ApplicationMethods.switchToLatestDLGframe(driver, testcaseName);
				WebElement ele=ExplicitWaitUtil.waitForElementTobeActionable(driver, objects_locatorType_Transmittals.get("Tramsmittals-Send"), objects_objectLocator_Transmittals.get("Tramsmittals-Send"), Constants_TimeOuts.Element_TimeOut);
				isSendDisplayed=ElementPresence.isElementDisplayed(driver, objects_locatorType_Transmittals.get("Tramsmittals-Send"), objects_objectLocator_Transmittals.get("Tramsmittals-Send"), Constants_TimeOuts.sync_element_load);
				commonMethods.actionsdoubleClick(driver, ele);
				System.out.println("----------Send recovery "+recoveryCounter+1+" -------------");
			}
	...
	public static void clickSubmit(WebDriver driver,String workFlow){
		...
		WaitUtil.pause(6);
		ApplicationMethods.waitForSpinnerToDisappear(driver, ObjRepository.spinner_pleasewait);
	}
	...
	public static void clickCompleteAction(WebDriver driver,String workFlow) throws Throwable{
		...
		ElementPresence.isElementDisplayed(driver, objects_locatorType_Transmittals_toolbar.get("Tramsmittals-Complete Action- Notification Link"), objects_objectType_Transmittals_toolbar.get("Tramsmittals-Complete Action- Notification Link"), Constants_TimeOuts.completeActionLink);
		
		res=KeyMethods.f_performAction(driver, refID, testcaseName, workFlow, "Tramsmittals-Complete Action- Notification Link", objects_locatorType_Transmittals_toolbar, objects_objectType_Transmittals_toolbar, objects_objectLocator_Transmittals_toolbar, input);
	 ...
	 * @throws Throwable
	 */
	public static Hashtable<String,String> forwardAndSendTransmittalRecord(String appName,WebDriver driver,String testcasename,String workFlow,Hashtable<String,String>data) throws Throwable{		
		Hashtable<String,String>returnData = new Hashtable<String,String>();
	 ...
	 * @throws Throwable
	 */
	public static void replyAllAndSendTransmittalRecord_old(String appName,WebDriver driver,String workFlow,Hashtable<String,String>data) throws Throwable{		
...
/*if(!data.get("IsConfidential").equalsIgnoreCase(Constants_FRMWRK.Tick)){
	...
	/*	int counter=1;
		List<WebElement>sendbuttons=null;
		WebElement element=null;
		//List<WebElement>sendbuttons=ExplicitWaitUtil.waitForPresenceOfElements(driver, Constants_FRMWRK.FindElementByXPATH, ".//*[@value='Send']", Constants_TimeOuts.Element_TimeOut);
		try{
			element=driver.findElement(By.xpath("(.//*[@value='Send'])[2]"));
			if(element!=null){
				System.out.println("Send button exists..");
			}else{
				System.out.println("Send not button exists..");
			}
		}catch (Exception ex){
		...
		try{
			...
			System.out.println("actionsClick2");
			commonMethods.actionsClick3(driver, element);
			System.out.println("actionsClick3");
			commonMethods.actionsdoubleClick(driver, element);
			System.out.println("actionsdouble Click");
		}catch(Exception ex){
			System.out.println("error"+ex);
		}
		
		for (WebElement elementSend : sendbuttons){
			commonMethods.actionsClick(driver, elementSend);
	...
	public static void attachdocument(String appName,WebDriver driver,String refid,String testcasename,String workflow,Hashtable<String,String>data) throws Throwable{
		...
		if(!data.get("AttachDocuments").isEmpty()){		
			if(!data.get("AttachDocuments").isEmpty()){
				...
				ApplicationMethods.waitForOverlayToDisappear(driver);
				ApplicationMethods.switchToLatestDLGframe(driver, testcasename);
				// wait until find an item displays
				...
				res=KeyMethods.f_performAction(driver, refid, testcasename, workflow, "Tramsmittals-Attached Document Files-Folder", objects_locatorType_Transmittals, objects_objectType_Transmittals, objects_objectLocator_Transmittals, input);
				WaitUtil.pause(Constants_TimeOuts.sync_frame_load);
				//ApplicationMethods.switchToLatestDLGframe(driver, testcasename);
...
//Attach
				if(!data.get("AttachDocumentName").contains(Constants.delimiter_data)){
					...
					while(counter<=3 && fileAttached==false){
						String[] docname=commonMethods.splitString(data.get("AttachDocumentName"), "\\.");
						WebTableUtil.searchforDataInsearchColumnAndClickInactionableLinkColumn(driver, testcaseName, workflow+" "+page+" - Select a Document", data.get("AttachDocuments"),docname[0] ,"Text", docname[0], 3, 7);
						fileAttached=ElementPresence.isElementDisplayed(driver, Constants_FRMWRK.FindElementByXPATH, objects_objectLocator_Transmittals.get("Tramsmittals-Attached Document Files-Remove"),Constants_TimeOuts.sync_element_load);
						counter=counter+1;
						if(counter==3){
							break;
						}
					...
					String[] getattachDocuments=commonMethods.splitString(data.get("AttachDocumentName"), Constants.delimiter_data);
...
//verify after attach
			...
			if(!data.get("AttachDocumentName").contains(Constants.delimiter_data)){	
				String[] docname=commonMethods.splitString(data.get("AttachDocumentName"), "\\.");
				String locator=objects_objectLocator_Transmittals.get("Tramsmittals-Attached Transmittal Files").replaceAll("docName", docname[0]);
				...
				String[] getattachDocuments=commonMethods.splitString(data.get("AttachDocumentName"), Constants.delimiter_data);
				String default_locator=objects_objectLocator_Transmittals.get("Tramsmittals-Attached Transmittal Files");
				...
				for (int doc=0;doc<getattachDocuments.length;doc++){
					...
					KeyMethods.f_fetchElementDetails(driver, refid, testcasename, workflow, "Tramsmittals-Attached Transmittal Files", objects_locatorType_Transmittals, objects_objectType_Transmittals, objects_objectLocator_Transmittals, docname[0]);
					objects_objectLocator_Transmittals.put("Tramsmittals-Attached Transmittal Files", default_locator);
				}
		...
		if(!data.get("AttachSupportDocuments").isEmpty()){
			...
			res=KeyMethods.f_performAction(driver, refid, testcasename, workflow, "Tramsmittals-Supported Document Files-Folder", objects_locatorType_Transmittals, objects_objectType_Transmittals, objects_objectLocator_Transmittals, input);
			WaitUtil.pause(Constants_TimeOuts.sync_frame_load);
...
//Attach Support docs		
			if(!data.get("AttachSupportDocumentName").contains(Constants.delimiter_data)){
				boolean fileAttached=false;
				int counter=0;
				//WebTableUtil.searchforDataInsearchColumnAndClickInactionableLinkColumn(driver, testcaseName, workflow+" "+page+" - Select a Document", ObjRepository.container_documentRegister, data.get("AttachSupportDocumentName"),"Text", data.get("AttachSupportDocumentName"), 3, 2);
				while(counter<=3 && fileAttached==false){
					String[] docname=commonMethods.splitString(data.get("AttachSupportDocumentName"), "\\.");
					WebTableUtil.searchforDataInsearchColumnAndClickInactionableLinkColumn(driver, testcaseName, workflow+" "+page+" - Select a Document", data.get("AttachSupportDocuments"), docname[0],"Text",docname[0] , 3, 7);
					fileAttached=ElementPresence.isElementDisplayed(driver, Constants_FRMWRK.FindElementByXPATH, objects_objectLocator_Transmittals.get("Tramsmittals-Attached Document Files-Remove"),Constants_TimeOuts.sync_element_load);
					counter=counter+1;
					if(counter==3){
						break;
				...
				String[] getattachDocuments=commonMethods.splitString(data.get("AttachSupportDocumentName"), Constants.delimiter_data);
...
//Attach Support docs-verify after attach	
			if(!data.get("AttachSupportDocumentName").contains(Constants.delimiter_data)){
				String[] docname=commonMethods.splitString(data.get("AttachSupportDocumentName"), "\\.");
				String locator=objects_objectLocator_Transmittals.get("Tramsmittals-Attached Support Document Files").replaceAll("docName", docname[0]);
				objects_objectLocator_Transmittals.put("Tramsmittals-Attached Support Document Files", locator);
				KeyMethods.f_fetchElementDetails(driver, refid, testcasename, workflow, "Tramsmittals-Attached Support Document Files", objects_locatorType_Transmittals, objects_objectType_Transmittals, objects_objectLocator_Transmittals, docname[0]);
				
			}else{
				
				String[] getattachDocuments=commonMethods.splitString(data.get("AttachSupportDocumentName"), Constants.delimiter_data);
				String default_locator=objects_objectLocator_Transmittals.get("Tramsmittals-Attached Support Document Files");
				...
				for (int doc=0;doc<getattachDocuments.length;doc++){
					...
					KeyMethods.f_fetchElementDetails(driver, refid, testcasename, workflow, "Tramsmittals-Attached Support Document Files", objects_locatorType_Transmittals, objects_objectType_Transmittals, objects_objectLocator_Transmittals, docname[0]);
					objects_objectLocator_Transmittals.put("Tramsmittals-Attached Support Document Files", default_locator);
				}				
	...
	public static void verifyAttachedFiles(String appName,WebDriver driver,String testcasename,String refid,String workflow,Hashtable<String,String>data) throws Throwable{
		if(data.get(Constants_Workflow.FluidTX_WorkFlow_Data_Condition).equalsIgnoreCase(Constants_Workflow.FluidTX_WorkFlow_IssuedForReview) && !data.get("ReviewDocument").isEmpty()){
			ApplicationMethods.switchToLatestDLGframe(driver, testcasename);
			WaitUtil.pause(Constants_TimeOuts.sync_frame_load);
			/* excelens change as per JIRA 510 
			...
			String locator=objects_objectLocator_Transmittals.get("Tramsmittals-View - Attached Review Document Files").replaceAll("docName", data.get("ReviewDocument"));
			*/
			String [] reviewsheetname=commonMethods.splitString(data.get("ReviewDocument"), "\\.");
			String locator=objects_objectLocator_Transmittals.get("Tramsmittals-View - Attached Review Document Files").replaceAll("docName", reviewsheetname[0]);
			
			objects_objectLocator_Transmittals.put("Tramsmittals-View - Attached Review Document Files", locator);
			KeyMethods.f_fetchElementDetails(driver, refid, testcasename, workflow, "Tramsmittals-View - Attached Review Document Files", objects_locatorType_Transmittals, objects_objectType_Transmittals, objects_objectLocator_Transmittals, reviewsheetname[0]);
		...
		if(!data.get("AttachDocuments").isEmpty()){	
			...
			if(!data.get("AttachDocumentName").contains(Constants.delimiter_data)){
				...
				String[] getattachDocuments=commonMethods.splitString(data.get("AttachDocumentName"), Constants.delimiter_data);
				String default_locator=objects_objectLocator_Transmittals.get("Tramsmittals-View -Transmittal Files");
				for (int doc=0;doc<getattachDocuments.length;doc++){
					String locator=objects_objectLocator_Transmittals.get("Tramsmittals-View -Transmittal Files").replaceAll("docName",getattachDocuments[doc]);
					objects_objectLocator_Transmittals.put("Tramsmittals-View -Transmittal Files", locator);
					
					KeyMethods.f_fetchElementDetails(driver, refid, testcasename, workflow, "Tramsmittals-View -Transmittal Files", objects_locatorType_Transmittals, objects_objectType_Transmittals, objects_objectLocator_Transmittals, getattachDocuments[doc]);
					objects_objectLocator_Transmittals.put("Tramsmittals-View -Transmittal Files", default_locator);
				}	
			}
		...
		if(!data.get("AttachSupportDocuments").isEmpty()){
			...
			if(!data.get("AttachSupportDocumentName").contains(Constants.delimiter_data)){
				...
				String[] getattachDocuments=commonMethods.splitString(data.get("AttachSupportDocumentName"), Constants.delimiter_data);
				String default_locator=objects_objectLocator_Transmittals.get("Tramsmittals-View -Supporting Document Files");
				
				for (int doc=0;doc<getattachDocuments.length;doc++){
					String locator=objects_objectLocator_Transmittals.get("Tramsmittals-View -Supporting Document Files").replaceAll("docName",getattachDocuments[doc]);
					objects_objectLocator_Transmittals.put("Tramsmittals-View -Supporting Document Files", locator);
					
					KeyMethods.f_fetchElementDetails(driver, refid, testcasename, workflow, "Tramsmittals-View -Supporting Document Files", objects_locatorType_Transmittals, objects_objectType_Transmittals, objects_objectLocator_Transmittals, getattachDocuments[doc]);
					objects_objectLocator_Transmittals.put("Tramsmittals-View -Supporting Document Files", default_locator);
				}	
	...
	public static Hashtable<String,String> replyAllAndSendTransmittalRecord(String appName,WebDriver driver,String workFlow,Hashtable<String,String>data) throws Throwable{		
		clickReplyAll(driver, workFlow);
		WaitUtil.pause(Constants_TimeOuts.Save_TimeOut);
		Transmittals_EntryPage.switchToTramsmittalEditFrame(driver, refID, testcaseName, workFlow);
		commonMethods.pageLoadWait(driver);
		Hashtable<String,String>returnData = new Hashtable<String,String>();
		res=KeyMethods.f_performAction(driver, refID, testcaseName, workFlow, "Tramsmittals-ReplyAll", objects_locatorType_Transmittals, objects_objectType_Transmittals, objects_objectLocator_Transmittals, input);
		returnData.put("Tramsmittals-ReplyAll-Users", res);
		if (res.isEmpty())
		{
			CustomExceptions.Exit(testcaseName, "Tramsmittals-ReplyAll -Failure", "Please refer above details for more details");
		}
	...
	public static void validate_TransmittalsAssociatedActions(WebDriver driver,String testcasename,String workflow,Hashtable<String,String>transmittedData,Hashtable<String,String>data) throws Throwable{
		String actionUser;
		TransmittalsGridUtil.searchSubjectAndOpenRecord(driver,Constants_Workflow.page_myInbox, workflow, transmittedData.get("Tramsmittals-Subject"));
		ApplicationMethods.switchToLatestDLGframe(driver, testcasename);
		//KeyMethods.f_performAction(driver, refID, testcasename, workflow, "Click on Expandable Icon for Actions", Constants_FRMWRK.FindElementByXPATH, "Link", ".//*[@id='txActionListToggle' and @class='toggle-button down']", "");
		res=KeyMethods.f_performAction(driver, refID, testcaseName, workflow, "Tramsmittals-View -Actions Expandable Icon", objects_locatorType_Transmittals, objects_objectType_Transmittals, objects_objectLocator_Transmittals, input);
	
		if(res.equalsIgnoreCase(Constants_FRMWRK.False)){
			CustomExceptions.Exit(testcasename, workflow+"Actions Expandable Icon-Failure", "Due to above failure could not continue execution ,Please refer above details for more details");
		}
		
		if(data.get("Action-Level2").contains("Forward")){
			actionUser=Workflows.runtime_transmittalData.get("Tramsmittals-ForwardTo");
			TransmittalsGridUtil.actionsStatusforUser(driver, workflow, actionUser, Constants.actions_status_completed);
		}
		else if(data.get("Action-Level2").contains("ReplyAll")){
			actionUser=Workflows.runtime_transmittalData.get("Tramsmittals-ReplyAll-Users");
			TransmittalsGridUtil.actionsStatusforUser(driver, workflow, actionUser, Constants.actions_status_completed);
		}
		else{
			if(transmittedData.get("Tramsmittals-To").contains(Constants.delimiter_data)){
				String[] ip=commonMethods.splitString(transmittedData.get("Tramsmittals-To"), Constants.delimiter_data);
				for (int i=0;i<ip.length;i++){
					TransmittalsGridUtil.actionsStatusforUser(driver, workflow, ip[i], Constants.actions_status_completed);
				}
			}else{
				actionUser=transmittedData.get("Tramsmittals-To");
				TransmittalsGridUtil.actionsStatusforUser(driver, workflow, actionUser, Constants.actions_status_completed);
			}

		}	
		ApplicationMethods.closeAllDialogswithClose(driver, refID, testcasename);
